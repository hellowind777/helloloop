...
test("任务 prompt 在缺少项目约束时注入内建安全底线与默认工程约束", () => {
  ...
  assert.match(prompt, /## 强制编码与产出基线/);
  assert.match(prompt, /代码体积控制：文件\/类超过 300 行/);
  assert.match(prompt, /新增公共函数必须写简洁 docstring/);
  assert.match(prompt, /## 默认工程约束（文档未明确时生效）/);
  assert.match(prompt, /必须一次性完成本轮应交付的全部工作/);
  assert.match(prompt, /禁止添加“如果你要我可以继续”/);
...
test("任务 prompt 在存在项目约束时保留自定义约束并继续附带安全底线", () => {
  ...
  assert.match(prompt, /## 内建安全底线/);
  assert.match(prompt, /## 强制编码与产出基线/);
  assert.match(prompt, /代码是唯一事实源/);
  assert.match(prompt, /## 项目\/用户约束/);
  assert.match(prompt, /必须沿用现有模块边界/);
});

test("任务 prompt 会强制禁止拆单交付与套话收尾", () => {
  const prompt = buildTaskPrompt({
    task: sampleTask(),
    verifyCommands: ["npm test"],
  });

  assert.match(prompt, /不要做半成品后停下来问“是否继续”/);
  assert.match(prompt, /不要把“下一步建议”当成提前停下的理由/);
  assert.match(prompt, /禁止使用“如果你要”/);
  assert.match(prompt, /“希望这对你有帮助”/);
});
...
test("分析 prompt 会附带安全底线并限制 constraints 只写项目特有约束", () => {
  ...
  assert.match(prompt, /Windows 环境禁止使用 cmd/);
  assert.match(prompt, /## 强制编码与产出基线/);
  assert.match(prompt, /文件\/类超过 400 行/);
  assert.match(prompt, /## 默认工程约束（文档未明确时也必须遵守）/);
  assert.match(prompt, /`constraints` 只写从项目文档或现有项目配置中提炼出的项目特有约束/);
  assert.match(prompt, /禁止再次读取 `~\/\.helloagents\/helloagents\.json`/);
  assert.match(prompt, /可选文件不存在时直接跳过/);
  assert.match(prompt, /初始化示例任务，只能视为占位模板/);
});
