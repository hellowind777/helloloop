...
test("run-once 执行阶段遇到 429 限流时会切换到其他引擎继续当前任务", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "helloloop-runtime-fallback-"));
  const binDir = path.join(tempRoot, "bin");
  const tempRepo = createDemoRepo(tempRoot);
  const task = sampleTask("finish-main-flow", "实现主业务流程");
  ...
  const claudeStateFile = createSequencedAgentCli(binDir, "claude", {
    versionText: "claude 2.1.87\n",
    analyzeSequence: [
      { payload: sampleReviewPayload(task.title) },
      { payload: sampleAnalysisPayload([]) },
    ...
    executeSequence: [
      { finalMessage: "Claude 已接手完成主业务流程。" },
    ],
  });
  createUnavailableCli(binDir, "gemini");
  ...
  writeJson(path.join(tempRepo, ".helloloop", "policy.json"), {
    ...
    claude: {
      model: "",
      executable: cliExecutable(binDir, "claude"),
  ...
  try {
    ...
    const claudeState = readJson(claudeStateFile);
    const backlog = readJson(path.join(tempRepo, ".helloloop", "backlog.json"));
    assert.equal(codexState.analyze, 0);
    assert.equal(codexState.execute, 1);
    assert.equal(claudeState.analyze, 1);
    assert.equal(claudeState.execute, 1);
